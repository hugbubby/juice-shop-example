...
export class AdministrationComponent implements OnInit {
  ...
  findAllUsers () {
    this.userService.find().subscribe((users) => {
      ...
      for (const user of this.userDataSource) {
        user.email = this.safelyRenderEmail(user.email, this.doesUserHaveAnActiveSession(user))
      }
  ...
  safelyRenderEmail(email: string, isActive: boolean): SafeHtml {
    const sanitizedEmail = this.sanitizer.sanitize(1, email) || '';
    const className = isActive ? 'confirmation' : 'error';
    return this.sanitizer.bypassSecurityTrustHtml(`<span class="${className}">${sanitizedEmail}</span>`);
  }
